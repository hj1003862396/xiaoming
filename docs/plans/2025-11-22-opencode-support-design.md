...
Add full xiaoming support for OpenCode.ai using a native OpenCode plugin architecture that shares core functionality with the existing Codex implementation.

## Background

OpenCode.ai is a coding agent similar to Claude Code and Codex. Previous attempts to port xiaoming to OpenCode (PR #93, PR #116) used file-copying approaches. This design takes a different approach: building a native OpenCode plugin using their JavaScript/TypeScript plugin system while sharing code with the Codex implementation.
...
2. **Platform-Specific Wrappers**
   - Codex: CLI script (`.codex/xiaoming-codex`)
   - OpenCode: Plugin module (`.opencode/plugin/xiaoming.js`)

3. **Skill Directories**
   - Core: `~/.config/opencode/xiaoming/skills/` (or installed location)
   - Personal: `~/.config/opencode/skills/` (shadows core skills)

### Code Reuse Strategy

Extract common functionality from `.codex/xiaoming-codex` into shared module:
...
```javascript
{
  ...
  schema: z.object({
    skill_name: z.string().describe('Name of skill (e.g., "xiaoming:xiaoming-brainstorming")')
  }),
...
1. **Inject xiaoming-using-xiaoming content**
   - Full content of the xiaoming-using-xiaoming skill
...
```javascript
// .opencode/plugin/xiaoming.js
const skillsCore = require('../../lib/skills-core');
const path = require('path');
const fs = require('fs');
const { z } = require('zod');

export const XiaomingPlugin = async ({ client, directory, $ }) => {
  const xiaomingDir = path.join(process.env.HOME, '.config/opencode/xiaoming');
  const personalDir = path.join(process.env.HOME, '.config/opencode/skills');

  return {
    'session.started': async () => {
      const usingXiaoming = await readSkill('xiaoming-using-xiaoming');
      const skillsList = await findAllSkills();
      const toolMapping = getToolMappingInstructions();

      return {
        context: `${usingXiaoming}\n\n${skillsList}\n\n${toolMapping}`
      };
    },
...
├── .codex/
│   ├── xiaoming-codex        # UPDATED: Use skills-core
│   ├── xiaoming-bootstrap.md
│   └── INSTALL.md
├── .opencode/
│   ├── plugin/
│   │   └── xiaoming.js       # NEW: OpenCode plugin
...
1. Create `lib/skills-core.js`
   - Extract frontmatter parsing from `.codex/xiaoming-codex`
...
2. Update `.codex/xiaoming-codex` to use shared core
...
1. Create `.opencode/plugin/xiaoming.js`
