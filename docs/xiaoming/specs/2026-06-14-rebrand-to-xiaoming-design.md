...
2. **命令绑定 `/xiaoming`**：将 `skills/brainstorming/` 目录重命名为 `skills/xiaoming/`，以实现用户输入 `/xiaoming` 时直接触发设计风暴。
3. **静默引导技能重构**：将原本用于引导初始化的 `using-xiaoming-bootstrap` 技能重构为 `using-xiaoming-bootstrap`。
...
在文件系统中进行以下移动和更名：
1. **技能目录重命名**：
   * `skills/brainstorming/` $\rightarrow$ `skills/xiaoming/` (注册 `/xiaoming` 命令)
   * `skills/using-xiaoming-bootstrap/` $\rightarrow$ `skills/using-xiaoming-bootstrap/` (静默引导)
2. **文档目录重命名**：
   * `docs/xiaoming/` $\rightarrow$ `docs/xiaoming/`
3. **GEMINI.md 更新**：
   * 指向 `@./skills/using-xiaoming-bootstrap/SKILL.md` 和相应工具文档。
...
* **`skills/xiaoming/SKILL.md`** (原 brainstorming):
  `description: "在进行任何创造性工作（如创建特性、构建组件、添加功能或修改行为）之前，您必须使用此技能。在实施前探索用户意图、需求和设计。"`
* **`skills/using-xiaoming-bootstrap/SKILL.md`** (原 using-xiaoming-bootstrap):
...
* **特定替换**：
  * 将对 `xiaoming:xiaoming` 的调用均改为 `xiaoming:xiaoming`。
  * 将对 `xiaoming:using-xiaoming-bootstrap` 的调用均改为 `xiaoming:using-xiaoming-bootstrap`。
...
* 调用指令汉化：“以下是你的 'xiaoming:using-xiaoming-bootstrap' 技能的完整内容...”
...
1. **结构验证**：确认 `skills/xiaoming` 和 `skills/using-xiaoming-bootstrap` 目录存在，物理目录 `docs/xiaoming/` 正常迁移。
2. **触发验证**：在 Cursor 插件加载后，测试输入 `/xiaoming` 能够正常拉起设计风暴对话。
3. **初始化验证**：开启新会话时，确保 Hook 能正常加载且注入的 `additional_context` 包含正确的中文提示语和 `using-xiaoming-bootstrap` 内容。
