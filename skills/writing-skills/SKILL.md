...
**必读背景：** 在使用本技能 (skill) 之前，你必须理解 xiaoming:test-driven-development。该技能定义了基本的红-绿-重构循环。本技能将 TDD 适配到文档领域。
...
只使用技能名，带明确的要求标记：
- ✅ 好：`**必须使用子技能：** 使用 xiaoming:test-driven-development`
- ✅ 好：`**必读背景：** 你必须理解 xiaoming:systematic-debugging`
...
**必读背景：** xiaoming:test-driven-development 技能解释了为什么这很重要。相同原则适用于文档。
...
**部署：**
- [ ] 将技能提交 (commit) 到 git 并推送到你的 fork（如果已配置）
- [ ] 考虑通过 PR 贡献回去（如果普遍有用）
