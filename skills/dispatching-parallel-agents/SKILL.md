---
name: dispatching-parallel-agents
description: "当面临两个或以上可在没有共享状态或顺序依赖关系的情况下独立进行的任务时使用"
---

# 派发并行代理 (Dispatching Parallel Agents)

## 概述

你将任务委托给拥有独立上下文的专用代理 (agent)。通过精确设计它们的指令和上下文，你确保它们保持专注并在任务中取得成功。它们永远不应继承你会话的上下文或历史——你构建好它们所需的一切。这同时也保留了你自己的上下文，用于协调工作。

当你有多个不相关的失败（不同测试文件、不同子系统、不同 bug），顺序排查会浪费时间。每项排查都是独立的，可以并行进行。

**核心原则：** 每个独立问题域派发一个代理 (agent)。让它们并发工作。

## 使用时机

```dot
digraph when_to_use {
    "多个失败？" [shape=diamond];
    "它们是独立的吗？" [shape=diamond];
    "单个代理排查所有问题" [shape=box];
    "每个问题域一个代理" [shape=box];
    "它们可以并行工作吗？" [shape=diamond];
    "顺序代理" [shape=box];
    "并行派发" [shape=box];

    "多个失败？" -> "它们是独立的吗？" [label="是"];
    "它们是独立的吗？" -> "单个代理排查所有问题" [label="否 - 相关联"];
    "它们是独立的吗？" -> "它们可以并行工作吗？" [label="是"];
    "它们可以并行工作吗？" -> "并行派发" [label="是"];
    "它们可以并行工作吗？" -> "顺序代理" [label="否 - 共享状态"];
}
```

**适用情形：**
- 3 个以上测试文件失败，且根本原因 (root cause) 各不相同
- 多个子系统各自独立故障
- 每个问题无需依赖其他问题的上下文即可理解
- 各项排查之间无共享状态

**不适用情形：**
- 失败之间存在关联（修一个可能修好其他）
- 需要理解完整的系统状态
- 代理之间会相互干扰

## 模式

### 1. 识别独立问题域

按故障内容分组：
- 文件 A 的测试：工具审批流程
- 文件 B 的测试：批量完成行为
- 文件 C 的测试：中止功能

每个问题域是独立的——修复工具审批不影响中止测试。

### 2. 创建聚焦的代理任务

每个代理获得：
- **明确的范围：** 一个测试文件或子系统
- **清晰的目标：** 让这些测试通过
- **约束：** 不要修改其他代码
- **预期输出：** 你发现了什么和修复了什么的摘要

### 3. 并行派发

```typescript
// 在 Claude Code / AI 环境中
Task("修复 agent-tool-abort.test.ts 失败")
Task("修复 batch-completion-behavior.test.ts 失败")
Task("修复 tool-approval-race-conditions.test.ts 失败")
// 三个任务并发运行
```

### 4. 审查与整合

代理返回时：
- 阅读每个摘要
- 验证修复之间不冲突
- 运行完整测试套件
- 整合所有变更

## 代理提示结构

好的代理提示：
1. **聚焦** — 一个清晰的问题域
2. **自包含** — 理解问题所需的所有上下文
3. **明确输出** — 代理应返回什么？

```markdown
修复 src/agents/agent-tool-abort.test.ts 中的 3 个失败测试：

1. "should abort tool with partial output capture" - 期望消息中包含 'interrupted at'
2. "should handle mixed completed and aborted tools" - 快速工具被中止而非完成
3. "should properly track pendingToolCount" - 期望 3 个结果但得到 0

这些是时序/竞态条件问题。你的任务：

1. 阅读测试文件，理解每个测试验证的内容
2. 识别根本原因 (root cause) - 时序问题还是实际 bug？
3. 修复方式：
   - 将任意超时替换为基于事件的等待
   - 如果发现中止实现中的 bug，进行修复
   - 如果测试行为已变更，调整测试预期

不要只是增加超时时间——找到真正的问题。

返回：你发现了什么以及修复了什么的摘要。
```

## 常见错误

**❌ 范围过宽：** "修复所有测试" — 代理会迷失
**✅ 具体明确：** "修复 agent-tool-abort.test.ts" — 聚焦的范围

**❌ 缺乏上下文：** "修复竞态条件" — 代理不知道在哪里
**✅ 提供上下文：** 粘贴错误信息和测试名称

**❌ 没有约束：** 代理可能重构所有内容
**✅ 有约束：** "不要修改生产代码" 或 "只修复测试"

**❌ 模糊的输出：** "修好它" — 你不知道改了什么
**✅ 具体明确：** "返回根本原因 (root cause) 和变更的摘要"

## 何时不使用

**相关联的失败：** 修一个可能修好其他——先一起排查
**需要完整上下文：** 理解需要看到整个系统
**探索性调试：** 你还不知道什么出了问题
**共享状态：** 代理会相互干扰（编辑相同文件、使用相同资源）

## 来自实际会话的真实示例

**场景：** 大规模重构后，3 个文件中出现 6 个测试失败

**失败情况：**
- agent-tool-abort.test.ts：3 个失败（时序问题）
- batch-completion-behavior.test.ts：2 个失败（工具未执行）
- tool-approval-race-conditions.test.ts：1 个失败（执行次数 = 0）

**决策：** 独立问题域——中止逻辑与批量完成与竞态条件各自独立

**派发：**
```
代理 1 → 修复 agent-tool-abort.test.ts
代理 2 → 修复 batch-completion-behavior.test.ts
代理 3 → 修复 tool-approval-race-conditions.test.ts
```

**结果：**
- 代理 1：将超时替换为基于事件的等待
- 代理 2：修复事件结构 bug（threadId 位置错误）
- 代理 3：添加等待异步工具执行完成的逻辑

**整合：** 所有修复相互独立，无冲突，完整测试套件全部通过

**节省时间：** 3 个问题并行解决，而非顺序执行

## 核心优势

1. **并行化** — 多项排查同时进行
2. **聚焦** — 每个代理范围窄，追踪的上下文更少
3. **独立性** — 代理之间互不干扰
4. **速度** — 1 份时间解决 3 个问题

## 验证

代理返回后：
1. **审查每个摘要** — 理解发生了什么变更
2. **检查冲突** — 代理是否编辑了相同的代码？
3. **运行完整套件** — 验证所有修复协同工作
4. **抽查** — 代理可能会犯系统性错误

## 真实影响

来自调试会话（2025-10-03）：
- 3 个文件中 6 个失败
- 并行派发 3 个代理
- 所有排查并发完成
- 所有修复成功整合
- 代理变更之间零冲突
